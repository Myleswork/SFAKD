...
class ShuffleNetV2_test(nn.Module):
    def __init__(self, net_size, num_classes=10):
        super(ShuffleNetV2_test, self).__init__()
        out_channels = configs[net_size]["out_channels"]
        num_blocks = configs[net_size]["num_blocks"]

        self.conv_test = nn.Conv2d(out_channels[3], num_classes, kernel_size=1, padding=0, bias=False)
