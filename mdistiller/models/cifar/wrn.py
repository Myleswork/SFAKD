...
class WideResNet_test(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0):
        super(WideResNet_test, self).__init__()
        nChannels = [16, 16 * widen_factor, 32 * widen_factor, 64 * widen_factor]
        assert (depth - 4) % 6 == 0, "depth should be 6n+4"
        n = (depth - 4) // 6
        block = BasicBlock
        ...
        self.conv_test = nn.Conv2d(nChannels[3], num_classes, kernel_size=1, padding=0, bias=False)
...
def wrn_40_2(**kwargs):
    ...
    return model
def wrn_40_2_test(**kwargs):
    model = WideResNet_test(depth=40, widen_factor=2, **kwargs)
    return model

def wrn_40_1(**kwargs):
    model = WideResNet(depth=40, widen_factor=1, **kwargs)
    return model
def wrn_40_1_test(**kwargs):
    model = WideResNet_test(depth=40, widen_factor=1, **kwargs)
    return model

def wrn_16_2(**kwargs):
    model = WideResNet(depth=16, widen_factor=2, **kwargs)
    return model
def wrn_16_2_test(**kwargs):
    model = WideResNet_test(depth=16, widen_factor=2, **kwargs)
    return model
