...
class ShuffleNet_test(nn.Module):
    def __init__(self, cfg, num_classes=10):
        super(ShuffleNet_test, self).__init__()
        out_planes = cfg["out_planes"]
        num_blocks = cfg["num_blocks"]
        groups = cfg["groups"]

        self.conv_test = nn.Conv2d(out_planes[2], num_classes, kernel_size=1, padding=0, bias=False)
        self.conv1 = nn.Conv2d(3, 24, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(24)
        self.in_planes = 24
...
def ShuffleV1_test(**kwargs):
    cfg = {"out_planes": [240, 480, 960], "num_blocks": [4, 8, 4], "groups": 3}
    return ShuffleNet_test(cfg, **kwargs)
