...
class MobileNetV2_test(nn.Module):
    ...
    def __init__(self, T, feature_dim, input_size=32, width_mult=1.0, remove_avg=False):
        super(MobileNetV2_test, self).__init__()
        self.remove_avg = remove_avg
        ...
        self.last_channel = int(1280 * width_mult) if width_mult > 1.0 else 1280
        self.conv_test = nn.Conv2d(self.last_channel, feature_dim, kernel_size=1, padding=0, bias=False)
        self.conv2 = conv_1x1_bn(input_channel, self.last_channel)
...
def mobile_half_test(num_classes):
    return mobilenetv2_T_w_test(6, 0.5, num_classes)
