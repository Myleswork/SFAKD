...
def reduce(train_set, rate=1, class_num=100, num_per_class = 500):
...
def get_cifar100_train_transform_aug():
...
def get_cifar100_dataloaders(batch_size, val_batch_size, num_workers,if_Augment, REDUCTION=False,
                             reduction_rate = 1., RESERVED_CLASS_NUM=100, auto_aug=False, cut_out=False):
    ...
    #----train set data reduction------
    if REDUCTION:
        train_set = reduce(train_set, rate=reduction_rate, class_num=RESERVED_CLASS_NUM, num_per_class = 500)
        print('reserved training set ratio: '+str(reduction_rate))
    #------------------------

    num_data = len(train_set)
    print('training set lenth: '+str(num_data))

    test_set = datasets.CIFAR100(
        root=data_folder, download=True, train=False, transform=test_transform
    )

    #----test set data reduction------
    # test_set = reduce(test_set, 1., class_num=99, num_per_class=100)
    # print('test set lenth: '+str(len(test_set)))
    #------------------------
